	...
	mounted () {
		...
		let _options = {
			...
			select: function (event, data) {
				...
				self.$emit('select', s)
				self.$emit('selectedRoot', selectedRootNodes) // consignment specific code

			},

			click : function (event, data){
				console.log('click');
				let id =$(data.originalEvent.target).attr('id');
				console.log('id', id);
				if(id != undefined && id != null && id != '' && (id == 'add' || id=="remove-1" || id=="remove-2")){
					let dataObj ={
						type: id,
						data: data.node
					}
					self.$emit('addOrRemove', dataObj);
				}
			},
		};
