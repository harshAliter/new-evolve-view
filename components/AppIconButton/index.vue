<template>
	<a :href="url" :class="styles" @click="handleClick">
		<i v-if="icon" class="material-icons icon" :style="'color:'+color">{{ icon }}</i>
		<img v-else :src="src" class="icon">

		<span v-if="badge" class="badge">{{ badge }}</span>
		<span v-if="tooltip && active === false" class="tooltip-content">{{ truncateString(tooltip) }}</span>
...
.app-icon-button.filled.primary .icon{
    color: #fff !important;
}
</style>