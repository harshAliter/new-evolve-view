<template>
	<div class="uk-grid fixed-footer">
		...
		<div class="uk-width-1-2@m uk-width-expand pagination">
			...
			<span v-if="isCountMode == 1 && resultLength > 0 " class="font-style-span"> {{ startFrom + 1 }} - {{ startFrom + resultLength }} of {{ totalRecords }} </span>	
			...
			<select v-if="isCountMode == '0' && resultLength > 0" v-model="isCountMode" class="styled-select" @change="onCalculateCount($event)">
				<option :key="0" value="0" selected>
					<span> {{ `${startFrom + 1} - ${ startFrom + resultLength } of Many` }}</span>		
				</option>
...
export default {
	...
	methods: {
		...
		onRefreshPage () {
			this.$emit('updatePage', 'refresh', (String(this.isCountMode) == '1'));
		},
